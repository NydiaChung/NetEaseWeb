// 写入cookie（存储在用户客户端计算机上的数据）expires：过期日期
function setCookie (name, value, expires, path, domain, secure) {
    var cookie = encodeURIComponent(name) + '=' + encodeURIComponent(value);
    if (expires instanceof Date)			//判断左边是否为右边的实例
        cookie += '; expires=' + expires.toGMTString();	//toGMTString()：根据格林威治时间 (GMT) 把 Date 对象转换为字符串，并返回结果
    if (path)
        cookie += '; path=' + path;
    if (domain)
...
function getcookie (name) {
    var cookieName=encodeURIComponent(name)+"=",		//用于检索指定cookie
...
// 导航关注，使用ajax
function attention(){
    var xhr=new XMLHttpRequest();	//后台与服务器交换数据，可以在不重新加载整个网页的情况下，对网页的某部分进行更新
     xhr.onreadystatechange=function(){	//onreadystatechange：存储函数（或函数名），每当 readyState 属性改变时，就会调用该函数。
    ...
    xhr.open("get",'http://study.163.com/webDev/attention.htm',true);	//获取数据
    xhr.send(null);
}
// 取消关注，删除cookie
function attCancel(){
...
function submit(){
        ...
        var rightname='studyOnline',	//账户
        rightpswd='study.163.com';	//密码
        // 表单验证
        if(name!=rightname){
